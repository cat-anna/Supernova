...
static void vmemwr(unsigned dst_off, unsigned char *src, unsigned count)
{
	...
	memcpy((void*)(get_fb_seg() * 16 + dst_off), src, count);
}
/*****************************************************************************
*****************************************************************************/
...
/* draw 2-color X */
	/*for(y = 0; y < g_ht; y++)
	{
		g_write_pixel((g_wd - g_ht) / 2 + y, y, 4);
		g_write_pixel((g_ht + g_wd) / 2 - y, y, 3);
	}*/
	g_write_pixel(319, 199, 1);
	for(y = 10, x = 0;x<256;){
		g_write_pixel(x, y, x);
	//	g_write_pixel(x+50, y+40, x);
	//	g_write_pixel(x*3+2, y, x);
		y++;
		if(y >= 20){
			x++;
			y=0;
